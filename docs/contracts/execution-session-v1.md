...
This contract defines who may use a local execution surface, how a Character session progresses, and how local safety behaves across contention, cancellation, disconnect, human presence, and human takeover.
...
1. physical human/local owner control, explicit human Stop / Take Over;
2. Local safety/permission/device-availability policy;
...
- denied when policy, permission, risk, human-presence, device mode, or safety does not permit execution.
...
Physical human/local-owner control and Local safety supersede Character scheduling. Among otherwise-valid Character/user intents requiring the same exclusive resource, scheduler priority is:
...
Higher Character/user priority does not normally mean arbitrary hard interruption.
...
Immediate hard interruption is reserved for physical human takeover, explicit human Stop/Take Over, Local safety/security enforcement, permission revocation, device-mode restriction, or equivalent emergency conditions.
...
- `accepted` — Local validated current device/session/plugin/human-presence policy and reserved required startup resources.
...
A deferred autonomy/delegated intent is **not yet an Execution Session** merely because it is waiting for a lease or suitable human/device availability. The Cloud scheduler may keep it as a bounded pending intent until admitted or expired.
...
Human takeover also includes credible physical keyboard/pointer activity detected while autonomous interactive input authority is armed.

Human takeover:

- prevents new autonomous side effects immediately;
- immediately disarms autonomous keyboard/pointer generation when the interactive desktop is being physically used;
- transitions the session to `paused`, `interrupted`, or `stopping` according to Local safety and the current adapter safe boundary;
- releases or safely unwinds the interactive input lease;
...
Physical human input does not wait for Cloud or model approval before disarming automation.
...
Human takeover/device-mode changes still apply while Cloud is disconnected.
...
- Local device availability mode still permits continuation;
- current human-presence/interactive-session evidence permits continuation where required;
- Local safety policy permits continuation.

A `session_recovery` intent competes for resources under the Autonomy Policy scheduler. Reconnection does not silently preempt physical human control or a higher-priority explicit user request.

Otherwise the session stops safely.

## 10. Target focus, Windows session, and desktop input

Generic desktop input is a high-risk exclusive resource.

V1 interactive desktop control targets the current unlocked interactive Windows session. It does not assume background service-desktop or multi-user-session automation.
...
- Windows lock, sleep, interactive-user/session switch, or equivalent loss of safe interactive context disarms autonomous input and pauses/stops safely;
- credible physical human keyboard/pointer input immediately revokes autonomous interactive use under the Human Takeover rule;
- Adapter Lab manual input requires explicit time-bounded arming;
- Runtime automation additionally requires plugin permission + session policy + human/device availability + lease.

Non-interactive/headless/API Execution Surfaces may continue while the owner uses the desktop when they do not conflict with an owner-held resource and all applicable policy remains valid.
...
- progress counters;
- interruption/failure reason codes, including human takeover where appropriate;
...
If the physical owner begins using keyboard/pointer input, both Character scheduler priority and any existing autonomous desktop-input lease yield to Local human takeover semantics.

A higher-priority explicit user request may cause cooperative preemption of a lower-priority autonomous session at a safe checkpoint, but it does not silently seize an actively human-used desktop.
