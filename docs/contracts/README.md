...
3. [`execution-session-v1.md`](execution-session-v1.md) — Character session, resource lease, disconnect, and takeover semantics.
4. [`plugin-permission-v1.md`](plugin-permission-v1.md) — Plugin/Adapter capability, permission, trust, and sandbox boundary.
5. current `character-relay` source/contracts for owner, Deployment, Presence, Portal, and durable Character evidence semantics.
...
`character-relay-local` is authoritative for the device transport, execution lease/session semantics, and Local Plugin Host permission boundary once the contracts are accepted.
...
A later implementation may refine field names, timing constants, library choices, or internal class layout without reopening Phase 0, provided it preserves these authority/security semantics.
...
- session/lease ownership or human-stop priority;
